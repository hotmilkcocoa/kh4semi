...
<%
	...
	List<BoardFreeDto> freelist;
	
		if(isSearch){
			freelist = boardfreeDao.pagingList(type,key, startRow, endRow);
		}
		else{
			freelist = boardfreeDao.pagingList(startRow, endRow);
		}
%>   
<%
 	//페이지 네비게이터 계산 코드를 작성
	...
	int count;
	if(isSearch){
		 count = boardfreeDao.getCount(type,key);
	}
	else{
		count = boardfreeDao.getCount();
	}
...
<script>
	$(function(){
	 ...
	 <div class="trans" align ="right" style="line-height:20px">
		<form action="free_list.jsp" method="post">
			<div class="row">
				<select name="type" class="input input-inline">
					<option value="free_title" <%if(type!=null&&type.equals("free_title")){%>selected<%}%>>제목</option>
					<option value="free_writer" <%if(type!=null&&type.equals("free_writer")){%>selected<%}%>>작성자</option>
				</select>
	...
	<div class="row">
		...
		<div class="write">
			...
			<div class="row">
				<ul class="pagination">
					<%if(isSearch){ %>
						<li><a href="free_list.jsp?p=<%=startBlock-1%>&type=<%=type%>&key=<%=key%>">&lt;</a></li>
					<%}else{ %>
						<li><a href="free_list.jsp?p=<%=startBlock-1%>">&lt;</a></li>
					<%} %>
					...
					<%} %>
						<%if(isSearch){ %>	
						<!-- 검색용 링크  -->
						<a href="free_list.jsp?p=<%=i%>&type=<%=type%>&key=<%=key%>"><%=i%></a>
						<%}else{ %>
						<!--  목록용 링크  -->
						<a href="free_list.jsp?p=<%=i%>"><%=i%></a>
						<%} %>
					<li>
					<%} %>
					
					<%if(isSearch){ %>
						<li><a href="free_list.jsp?p=<%=endBlock+1%>&type=<%=type%>&key=<%=key%>">&gt;</a></li>
					<%}else{ %>
						<li><a href="free_list.jsp?p=<%=endBlock+1%>">&gt;</a></li>
					<%} %>
