...
//수정기능
	public boolean update(BoardDto dto) throws Exception {
		...
		String sql = "update board "
						+ "set board_writer=?, board_header=?, board_title=?, board_context=?, board_writedate=?,"
						+ "board_dep=? where board_no=?";
		PreparedStatement ps = con.prepareStatement(sql);
		ps.setInt(1, dto.getBoard_writer());
		ps.setString(2, dto.getBoard_header());
		ps.setString(3, dto.getBoard_title());
		ps.setString(4, dto.getBoard_context());
		ps.setDate(5, dto.getBoard_writedate());
		ps.setString(6, dto.getBoard_dep());
		ps.setInt(7, dto.getBoard_no());
